...
export class TabbedZone extends Zone {
  public tabHeight: number = 33;
  public tabWidth: number = 200;
  public tabs: ZoneTab[] = [];

  public enableTabBar = true;
  public tabBar: StBoxLayout = new St.BoxLayout({ style_class: "tab-bar" });
  public tabsTop = false;

  get innerY(): number {
    if (this.tabs.length > 1 && this.tabsTop) {
  ...
  destroy() {
    ...
    if (this.enableTabBar) Main.uiGroup.remove_child(this.tabBar);
  }
  ...
  adjustWindows(windows: Window[]) {
    ...
    let y = this.tabsTop
      ? this.y + this.margin
      : this.y + this.height - this.tabHeight - this.margin;
    ...
    if (this.tabs.length < 2) {
      ...
      this.tabs = [];
      if (this.enableTabBar) Main.uiGroup.remove_child(this.tabBar);
    } else if (this.enableTabBar) {
      this.tabBar.x = this.x + this.margin;
      this.tabBar.y = y;
      this.tabBar.height = this.tabHeight - this.margin * 2;
      this.tabBar.width = this.innerWidth;
      Main.uiGroup.insert_child_above(this.tabBar, global.window_group);
    }
...
export class ZoneTab {
  ...
  constructor(
    ...
    this.buttonWidget.connect("button-press-event", () => {
      ...
      this.buttonWidget.style_class = "tab-button clicked";
    });
    tabBar.add_child(this.buttonWidget);
...
export class EditableZone extends Zone {
  ...
  positionChanged() {
    super.positionChanged();
    this.widget.label = `${toRoundedString(this.innerWidth)}x${toRoundedString(
      this.innerHeight
...
export class ZoneGroup extends ZoneBase {
  ...
  public split(zone: Zone) {
    if (
      zone.layoutItem.type === 0 &&
      Math.floor(zone.width / 2) < zone.minWidth
    )
      return;
    if (
      zone.layoutItem.type === 1 &&
      Math.floor(zone.height / 2) < zone.minHeight
    )
      return;
...
export class ZoneAnchor {
  ...
  public adjustSizes() {
    if (this.zoneGroup.layoutItem.type == 0) {
      this.widget.x = Math.floor(
        this.zoneA.x + this.zoneA.width - this.widget.width / 2
      );
      this.widget.y = Math.floor(
        this.zoneA.y + this.zoneA.height / 2 - this.widget.height / 2
      );
    } else {
      this.widget.y = Math.floor(
        this.zoneA.y + this.zoneA.height - this.widget.height / 2
      );
      this.widget.x = Math.floor(
        this.zoneA.x + this.zoneA.width / 2 - this.widget.width / 2
      );
    }
  }
...
export class ZoneManager extends ZoneDisplay {
  ...
  constructor(
    ...
    super(monitor, layout, margin);
    this._last_selection_zone = new SelectionZone(
      layout,
      this.parent,
      this.stage
    );
  ...
  public getSelectionRect(): Rectangle | undefined {
    ...
    if (!this._last_selection_zone.contains(x, y)) return undefined;
  ...
  public highlightZonesUnderCursor() {
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      zone.selected =
        contained || (this._select_multiple_zones && (zone as Zone).selected);
      if (!zone.selected) continue;

      if (zone.x < smallestX) smallestX = zone.x;
      if (zone.y < smallestY) smallestY = zone.y;

      if (zone.x + zone.width > biggestX) biggestX = zone.x + zone.width;
      if (zone.y + zone.height > biggestY) biggestY = zone.y + zone.height;
    }
  ...
  private trackCursorUpdates() {
    this.trackCursorTimeoutId = GLib.timeout_add(
